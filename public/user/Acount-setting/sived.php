            ...
            <div class="main">
                <div class="sect" id="accountSection">
                    ...
                    <div class="notification-msg">
                        <?php
                        $query = mysqli_query($conn, "SELECT * FROM saved,books WHERE saved.idBook=books.idBook AND saved.idUser='$idUser'");
                        // if (mysqli_num_rows($query) > 0)
                        {
                            ...
                            while ($row = mysqli_fetch_assoc($query)) {
                                ...
                                <p>
                                    ...
                                    // echo " : ";
                                    // echo $row['parallelTitele']; 
                                    ?>
                                </p>
                                <div class="noti" style="display: flex;">
                                    <form action="delete.php" method="post">
                                        <input type="hidden" name="idS" value="<?php echo $row['idS']; ?>">
                                        <button type="submit" id="btn-N" class="delet-btn" name="deletS"
                                            onclick="return confirm('are you sure you want to delete ?')">Delete</button>
                                    </form>
                                    <button id="btn-N" class="get-btn"><a
                                            href="../consulterBooks/viewBook.php?idBook=<?php echo $row['idBook']; ?>&idType=<?php echo $row['idType']; ?>&idCategorie=<?php echo $row['idCategorie']; ?>">View</a></button>
                                </div>
                                <br>
                        ...
                        $queryGP = mysqli_query($conn, "SELECT * FROM saved_gp,graduation_project WHERE saved_gp.idGP=graduation_project.idGP AND saved_gp.idUser='$idUser'");
                        // if (mysqli_num_rows($queryGP) > 0) 
                        {
                            ...
                            while ($row = mysqli_fetch_assoc($queryGP)) {
                                ...
                                <div class="noti" style="display: flex;">
                                    <form action="delete.php" method="post">
                                        <input type="hidden" name="idS" value="<?php echo $row['idSGP']; ?>">
                                        <button type="submit" id="btn-N" class="delet-btn" name="deletSGP"
                                            onclick="return confirm('are you sure you want to delete ?')">Delete</button>
                                    </form>
                                    <button id="btn-N" class="get-btn"><a href="../consulterBooks/viewGP.php?idGP=<?php echo $row['idGP']; ?>&idCategorie=<?php echo $row['idCategorie']; ?>">View</a></button>
                                </div>
                                <br>
