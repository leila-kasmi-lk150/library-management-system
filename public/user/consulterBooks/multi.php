	...
	<body>
		...
		<div class="body">
			<table>
				...
				// test if user borrow GP IF HE DID SO HE CAN'T BORROW BOOK
				$borrowGPtest = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM borrow_gp WHERE idUser='$idUser' AND isReturn='0'"));
				$requestGPtest = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM request_gp WHERE idUser='$idUser'"));
				$total_gp = $borrowGPtest + $requestGPtest;


				// test if the user borrow more then 2 books 
				...
				$total = $b['b'] + $r['r'];
				// ===== 
				$sql = mysqli_query($conn, "SELECT * FROM books WHERE idBook=$idBook");
				...
				if ($block['block'] == '1') {
					...
					?>
							<tr>
								<td>
									<h2>
										<center>
											you can't Get more than 2 books
										</center>
									</h2>
								</td>
							</tr>
							<tr>
								<td>
									<center>
										<button>
											<a
												href="viewBook.php?idCategorie=<?php echo $idCategorie; ?>&idBook=<?php echo $idBook; ?>&idType=<?php echo $idType; ?>">Cancel</a>
										</button>
									</center>
								</td>
							</tr>
					<?php
				} else if ($row['quantity'] <= 1 && $Action == "borrow") {
					$r = "read";
					?>
								<tr>
									<td>
										<h2>
											<center>
												This Book Cant't Be Borrowed , You Can just read it in library
											</center>
										</h2>
									</td>
								</tr>
								<tr>
									<td>
										<center>
											<button>
												<a
													href="multi.php?idCategorie=<?php echo $idCategorie; ?>&idBook=<?php echo $idBook; ?>&idType=<?php echo $idType; ?>&Action=<?php echo $r; ?>">Read</a>
											</button>
											<button>
												<a
													href="viewBook.php?idCategorie=<?php echo $idCategorie; ?>&idBook=<?php echo $idBook; ?>&idType=<?php echo $idType; ?>">Cancel</a>
									...
									<tr>
										<td>
											<center>
												<button>
													<a
														href="sendNotification.php?idBook=<?php echo $idBook ?>&idUser=<?php echo $idUser ?>">yes</a>
												</button>
												<button>
													<a
														href="viewBook.php?idCategorie=<?php echo $idCategorie; ?>&idBook=<?php echo $idBook; ?>&idType=<?php echo $idType; ?>">Cancel</a>
					...
					<?php
				} else if ($row['nbrCopy'] <= 1 && $row['quantity'] > 1) {
					?>
										<tr>
											<td>
												<h2>
										...
										<tr>
											<td>
												<center>
													<button>
														<a
															href="sendNotification.php?idBook=<?php echo $idBook ?>&idUser=<?php echo $idUser ?>&idCategorie=<?php echo $idCategorie; ?>&idType=<?php echo $idType; ?>">yes</a>
													</button>
													<button>
														<a
															href="viewBook.php?idCategorie=<?php echo $idCategorie; ?>&idBook=<?php echo $idBook; ?>&idType=<?php echo $idType; ?>">Cancel</a>
					...
					$request = mysqli_query($conn, "INSERT INTO request(idUser,idBook,Action) VALUES ('$idUser','$idBook','$Action')");
						...
						?>
											<tr>
												<td>
													<h2>
														<center>
															<h2>You Can Approach The Library To Take The Book <br>
